...
class ManualWeeklyOffManager {
  constructor() {
    // Updated: All Saturdays and Sundays are weekly off
    this.weekOffDays = {
      sunday: true,
      saturday: true // Changed from '2nd,4th' to true for all Saturdays
    };
  }
  ...
  shouldBeWeeklyOff(date) {
    // Updated: Return true for all Saturdays and Sundays
    return this.isSunday(date) || this.isSaturday(date);
  }
  ...
  async processWeeklyOffForDateRange(startDate, endDate, options = {}) {
    ...
    console.log(`📅 Policy: All Saturdays and Sundays will be marked as Weekly Off`);
    ...
    for (let currentDate = new Date(start); currentDate <= end; currentDate.setDate(currentDate.getDate() + 1)) {
      ...
      // Check if this date should be weekly off (Saturday or Sunday)
      if (!this.shouldBeWeeklyOff(currentDate)) {
        console.log(`⏭️  Skipping working day: ${dateStr} (${dayName})`);
        results.totalDatesProcessed++;
        continue;
      }

      console.log(`\n📅 Processing weekly off date: ${dateStr} (${dayName})`);
        ...
        try {
          ...
          if (!existingRecord) {
            ...
            if (createMissing && !dryRun) {
              const created = await this.createWeeklyOffRecord(employee.id, currentDate, dayName);
              if (created) {
                dateRecordsCreated++;
              ...
              console.log(`📝 Would create weekly off record for ${employee.employeeName} on ${dateStr} (${dayName})`);
              dateRecordsSkipped++;
      ...
      console.log(`✅ Completed ${dateStr} (${dayName}): ${dateRecordsCreated} created, ${dateRecordsUpdated} updated, ${dateRecordsSkipped} skipped`);
    }
  ...
  generateReport(results, startDate, endDate, options) {
    ...
    console.log(`🏖️  Weekly Off Dates Found (All Saturdays & Sundays): ${results.totalWeeklyOffDates}`);
    ...
    console.log('\n📅 DATE-WISE SUMMARY (Weekly Off Days Only):');
    ...
    if (employeeSummary.length > 0) {
      ...
      console.log('   No records were created or updated for any employee');
    }

    // Calculate weekly off days count
    const saturdays = results.dateWiseResults.filter(d => d.weekType === 'Saturday').length;
    const sundays = results.dateWiseResults.filter(d => d.weekType === 'Sunday').length;
    
    console.log('\n📅 WEEKLY OFF DAYS BREAKDOWN:');
    console.log(`   Saturdays: ${saturdays} days`);
    console.log(`   Sundays: ${sundays} days`);
    console.log(`   Total Weekly Off Days: ${saturdays + sundays} days`);
  }
}
...
async function runManualWeeklyOffProcessing(startDate, endDate, options = {}) {
  ...
  console.log('📅 Policy: Processing ALL Saturdays and Sundays as Weekly Off');
